...
public class CategoriaService {
    ...
    public CategoriaResponseDto salvarCategoria(CategoriaRequestDto dto) {
        ...
        if (categoria == null) {
            throw new IllegalArgumentException("Erro ao converter os dados da Categoria - Categoria Nula");
        }
    ...
    public void deletarCategoriaPorId(Long id, boolean confirm) {
        ...
        if (!categoria.getLivros().isEmpty() && confirm == false) {
            throw new IllegalArgumentException("A Categoria " + categoria.getNomeCategoria() + " - ID: " + id
                    + " possui " + categoria.getLivros().size()
                    + " livro(s). Para deletar tudo, envie o parâmetro de confirmação. (/categorias/id-categoria?confirm=true)");
        }
